"""hud_renderer.py - Clean overlay, no global alpha/grey mask anywhere

Fully preserves camera pixels, modifies ONLY visible line/text/effect pixels
...
class HUDRenderer:
    ...
    def draw_face_box(self, frame: np.ndarray, bbox: Tuple[int, int, int, int], color: Tuple[int, int, int] = None, thickness: int = 2, corner_radius: int = 22, glow: bool = True) -> np.ndarray:
        ...
        # Draw outline directly onto frame (no alpha overlay)
        # Straight lines
        ...
        # Subtle glow (draw slightly larger border, blur, and blend ONLY over visible border)
        if glow:
            glow_img = np.zeros_like(frame)
            cv2.line(glow_img, (x1 + r, y1), (x2 - r, y1), color, thickness+6)
            cv2.line(glow_img, (x1 + r, y2), (x2 - r, y2), color, thickness+6)
            cv2.line(glow_img, (x1, y1 + r), (x1, y2 - r), color, thickness+6)
            cv2.line(glow_img, (x2, y1 + r), (x2, y2 - r), color, thickness+6)
            cv2.ellipse(glow_img, (x1 + r, y1 + r), (r, r), 180, 0, 90, color, thickness+6)
            cv2.ellipse(glow_img, (x2 - r, y1 + r), (r, r), 270, 0, 90, color, thickness+6)
            cv2.ellipse(glow_img, (x1 + r, y2 - r), (r, r), 90, 0, 90, color, thickness+6)
            cv2.ellipse(glow_img, (x2 - r, y2 - r), (r, r), 0, 0, 90, color, thickness+6)
            blurred = cv2.GaussianBlur(glow_img, (27,27), 0)
            mask = blurred > 18  # Where glow is nonzero
            frame[mask] = cv2.addWeighted(frame[mask], 0.7, blurred[mask], 0.3, 0)
        return frame
    ...
    def draw_status_panel(self, frame: np.ndarray, status_lines: List[str], position: Tuple[int, int] = (12, 65), bg_alpha: float = 0.08) -> np.ndarray:
        # Very subtle, almost invisible background
        x, y = position
        panel_width = 340
        line_height = 20
